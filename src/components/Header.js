...
export default function Header() {
  ...
  return (
    <div className="flex  items-center  z-20 w-screen fixed md:absolute  py-1 bg-gradient-to-b from-black sm: md:flex-row px-4 md:px-8 justify-between ">
      ...
      {user && (
        <div className="flex md:flex items-center gap-2 ">
          ...
          <button className="text-white text-xs md:text-lg font-bold mr-2 md:mr-2">
          <Link to="/my-list">
          My List
             </Link>
          </button>
          ...
          <button
            className="text-xs font-bold md:text-lg p-0 mr-2 text-white md:mr-2 "
          ...
          {search && (
            ...
            >
              {LNG_TYPE.map((l) => {
                return (
                  <option key={l.name} value={l.lngCode}>
                    {l.name}
                  </option>
                );
              })}
